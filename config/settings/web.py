...
# Cloud public URL for browser redirects (SSO login, Stripe checkout)
CLOUD_PUBLIC_URL = config('CLOUD_BASE_URL', default=config('CLOUD_API_URL', default='https://erplora.com'))

# Cloud internal URL for server-to-server API calls (marketplace, heartbeat, etc.)
# Uses private network proxy to avoid Floating IP hairpin NAT issues
# Falls back to public URL if not set
CLOUD_API_URL = config('CLOUD_INTERNAL_URL', default=CLOUD_PUBLIC_URL)
